...
export function Splash({ onDone }: { onDone: () => void }) {
  ...
  return (
    ...
    >
      <img
        src={logo.url}
        alt="Jai Bharat Junior College"
        className="h-28 w-28 rounded-full bg-white object-cover shadow-2xl animate-pulse"
      />
      <h1 className="mt-6 text-white text-2xl font-bold text-center px-4">Jai Bharat Junior College</h1>
      <p className="mt-2 text-white/80 text-sm">Nakkalgutta, Hanamkonda</p>
    </div>
  );
}