        ...
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
          <Link to="/" className="flex items-center gap-3 min-w-0">
            <img
              src={logo}
              alt={COLLEGE_NAME}
              className="h-10 w-10 rounded-full bg-white object-cover shrink-0"
            />
          ...
          <nav className="hidden md:flex items-center gap-1">
            {NAV.map((n) => (
              <Link
                ...
                activeProps={{
                  className:
                    "bg-white/15 px-3 py-2 rounded-md text-sm font-medium",
                }}
      ...
      <nav className="md:hidden fixed bottom-0 inset-x-0 z-40 bg-card/95 backdrop-blur border-t border-border grid grid-cols-4">
        {BOTTOM_NAV.map((n) => (
          <Link
            ...
            activeProps={{
              className:
                "flex flex-col items-center justify-center py-2 text-[11px] text-primary font-semibold",
            }}
            activeOptions={{ exact: n.to === "/" }}
          >
            <n.icon size={20} />
        ...
        <div className="max-w-6xl mx-auto px-4 py-8 grid sm:grid-cols-3 gap-6 text-sm">
          <div>
            ...
            <p className="opacity-80">
              Empowering students with quality intermediate education.
            </p>
          </div>
